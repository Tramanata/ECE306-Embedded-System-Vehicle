...
unsigned volatile int bl_timer = 0;
unsigned volatile int blackline_timer = 0;
unsigned volatile int turn_in_timer = 0;
unsigned volatile int turn_in_flag = 0;

extern unsigned volatile int move_time;
extern unsigned volatile int start_circle_stop;
extern unsigned volatile int circle_stop;
extern unsigned volatile int ping_flag;
...
//--------------------------------------------------------------------
    ...
    if(milliseconds < 10){
        ...
        if(IOT_step_counter > 8 && run_bootup == 1 && step_temp < 6){
            IOT_step_counter = 0;
            IOT_step = step_temp++;
        }
    ...
    if(blackline_timer == 1){
        bl_timer++;
    }

    if(start_circle_stop == 1){
        circle_stop++;
    }
